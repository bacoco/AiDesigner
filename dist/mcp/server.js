...
async function initializeProject(projectPath = process.cwd()) {
    ...
    phaseTransitionHooks.bindDependencies({
        triggerAgent: async (agentId, context) => {
            ...
            if (typeof rawResponse === "string") {
                try {
                    ...
                    console.error(`[MCP] Failed to parse response from agent ${agentId}: ${error instanceof Error ? error.message : error}`);
                    return {
        ...
        loadPhaseContext: async (phase, context) => {
            return contextPreservation.preserveContext(projectState.state.currentPhase, phase, context, async (p) => projectState.getPhaseDeliverables(p));
        },
    });
}
        ...
        switch (name) {
            case "get_project_context": {
                ...
                if (params.includeConversation !== false) {
                    context.recentConversation = projectState.getConversation(params.conversationLimit || 10);
                }
                return {
                    content: [
                        {
            ...
            case "detect_phase": {
                ...
                const result = await phaseTransitionHooks.checkTransition({ conversation: params.conversationHistory || [] }, params.userMessage, projectState.state.currentPhase);
...
${agent.content}`,
            ...
            case "transition_phase": {
                ...
                const result = await phaseTransitionHooks.executeTransition(fromPhase, params.toPhase, params.context || {});
                return {
                    content: [
                        {
                            type: "text",
