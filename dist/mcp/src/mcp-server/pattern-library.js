...
exports.applyDesignSystem = applyDesignSystem;
exports.getPatternsForScreen = getPatternsForScreen;
exports.selectPatternByStyle = selectPatternByStyle;
exports.generateVariation = generateVariation;
exports.validatePattern = validatePattern;
/**
 * Apply design system to pattern HTML
 */
function applyDesignSystem(html, design) {
  let result = html;
  // Color replacements
  ...
  result = result.replace(
    /\{\{font-heading\}\}/g,
    design.typography.headingFont || design.typography.fontFamily,
  );
  result = result.replace(
    /\{\{font-body\}\}/g,
    design.typography.bodyFont || design.typography.fontFamily,
  );
  ...
  result = result.replace(
    /\{\{shadow-sm\}\}/g,
    design.components?.shadowScale?.[0] || '0 1px 2px rgba(0, 0, 0, 0.05)',
  );
  result = result.replace(
    /\{\{shadow\}\}/g,
    design.components?.shadowScale?.[1] || '0 4px 6px rgba(0, 0, 0, 0.1)',
  );
  result = result.replace(
    /\{\{shadow-lg\}\}/g,
    design.components?.shadowScale?.[2] || '0 10px 15px rgba(0, 0, 0, 0.15)',
  );
 ...
 */
function getPatternsForScreen(screenType) {
  return exports.patternLibrary[screenType] || exports.patternLibrary.dashboard;
}
/**
 * Select pattern based on style preference
 */
function selectPatternByStyle(patterns, style, index = 0) {
 ...
 */
function generateVariation(basePattern, variationType) {
 ...
 */
function validatePattern(html) {
  // Basic validation
  return html.includes('<!DOCTYPE') && html.includes('<html') && html.includes('</html>');
}
exports.default = exports.patternLibrary;
