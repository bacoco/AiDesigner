...
class CodexConfigManager {
  ...
  applyDefaults(config) {
    ...
    if (existingServer.type !== DEFAULT_MCP_SERVER.type) {
      existingServer.type = DEFAULT_MCP_SERVER.type;
      changed = true;
    }

    if (existingServer.command !== DEFAULT_MCP_SERVER.command) {
    ...
    if (
      ...
      !arraysEqual(existingServer.args, DEFAULT_MCP_SERVER.args)
    ) {
      existingServer.args = [...DEFAULT_MCP_SERVER.args];
      changed = true;
    }

    if (existingServer.auto_start !== DEFAULT_MCP_SERVER.auto_start) {
      existingServer.auto_start = DEFAULT_MCP_SERVER.auto_start;
      changed = true;
    }

    if (typeof existingServer.description !== 'string') {
