...
function selectLane(userMessage, context = {}) {
  ...
  const level = typeof scaleAssessment?.level === 'number' ? scaleAssessment.level : undefined;
  const levelRationale = buildScaleRationale(scaleAssessment);
...
/**
 * Builds a human-readable rationale summarizing the scale assessment.
 *
 * @param {Object} scaleAssessment - The scale assessment object containing level, score, and signals
 * @param {number} scaleAssessment.level - The scale level (e.g., 1-5)
 * @param {number} scaleAssessment.score - The numeric score
 * @param {Object} scaleAssessment.signals - Object containing contributions and deductions arrays
 * @param {Array} scaleAssessment.signals.contributions - Array of positive signal entries
 * @param {Array} scaleAssessment.signals.deductions - Array of negative signal entries
 * @returns {string|undefined} A formatted rationale string, or undefined if scaleAssessment is invalid
 *
 * @example
 * // Returns: "Positive signals: Quick fix keywords (+2). Result: level 1 (score 2)."
 * buildScaleRationale({
 *   level: 1,
 *   score: 2,
 *   signals: {
 *     contributions: [{ description: 'Quick fix keywords', value: 2 }],
 *     deductions: []
 *   }
 * });
 */
function buildScaleRationale(scaleAssessment) {
  if (!scaleAssessment || typeof scaleAssessment !== 'object') {
    return;
  }

  const { level, score, signals } = scaleAssessment;
  const contributions = Array.isArray(signals?.contributions) ? signals.contributions : [];
  const deductions = Array.isArray(signals?.deductions) ? signals.deductions : [];

  const formatEntry = (entry, signSymbol) => {
    if (!entry || typeof entry.description !== 'string') {
      return null;
    }

    const value = typeof entry.value === 'number' ? `${signSymbol}${Math.abs(entry.value)}` : null;
    return value ? `${entry.description} (${value})` : entry.description;
  };
  ...
  if (contributions.length > 0) {
    const contributionText = contributions
      ...
      .join(', ');
    if (contributionText) {
      parts.push(`Positive signals: ${contributionText}.`);
    }
  }
  ...
  if (deductions.length > 0) {
    const deductionText = deductions
      ...
      .join(', ');
    if (deductionText) {
      parts.push(`Negative signals: ${deductionText}.`);
    }
  }
  ...
  const levelText = typeof level === 'number' ? `level ${level}` : 'unknown level';
  const scoreText = typeof score === 'number' ? score : 'n/a';
  parts.push(`Result: ${levelText} (score ${scoreText}).`);
...
async function logDecision(projectPath, decision, userMessage) {
  ...
  const level = decision.level ?? decision.scale?.level;
  const levelRationale = decision.levelRationale ?? buildScaleRationale(decision.scale);
