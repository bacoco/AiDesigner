...
# Start chatting
npm run agilai:chat       # Prompts for assistant choice
npm run agilai:chat:claude    # Claude front-end
npm run agilai:chat:codex     # Codex front-end
npm run agilai:chat:opencode  # OpenCode front-end

# Use GLM provider
npm run agilai:chat -- --glm
npm run agilai:chat:claude -- --glm
```

> **Note**: The previous `npm run bmad*` commands have been retired. Update any scripts or aliases that depended on them.
...
npm run agilai:chat         # Prompts for choice
npm run agilai:chat:claude  # Claude CLI
npm run agilai:chat:codex   # Codex CLI
npm run agilai:chat:opencode # OpenCode CLI
```
...
3. Use explicit CLI flag: `npm run agilai:chat -- --glm` or `--anthropic`
