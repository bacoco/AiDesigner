...
- **Discovery surface** – V6 advertises tools under semantic identifiers such as `project.context` and expects the registry to include metadata, titles, and versioning fields rather than the flat MCP list our server currently returns.【F:.dev/src/mcp-server/v6-tool-adapter.ts†L60-L83】【F:.dev/src/mcp-server/v6-tool-adapter.ts†L137-L213】
- **Invocation envelope** – Requests arrive as `{ tool, version, payload, context }` instead of the MCP-native `{ name, arguments }` tuple. The adapter must lift context hints (phase, lane, project ID) into the argument bag before delegating to the legacy handler.【F:.dev/src/mcp-server/v6-tool-adapter.ts†L91-L114】
- **Response contract** – V6 expects `{ tool, data, error? }`, so we need to wrap MCP responses (or errors) accordingly before handing them back to the V6 orchestrator.【F:.dev/src/mcp-server/v6-tool-adapter.ts†L121-L135】

### 6.2. Adapter Strategy

We introduced `.dev/src/mcp-server/v6-tool-adapter.ts` to map between the two worlds. The module provides three core helpers:

1. `mapV6InvocationToLegacy` – Converts the V6 envelope into the legacy tool name plus normalized arguments, reusing upstream routing hints for lane/phase selection.【F:.dev/src/mcp-server/v6-tool-adapter.ts†L91-L114】
2. `mapLegacyResultToV6` – Packages MCP handler results or thrown errors into the V6 response format, preserving the tool identifier mapping for observability.【F:.dev/src/mcp-server/v6-tool-adapter.ts†L121-L135】
3. `projectLegacyToolsToV6Catalog` – Re-exports the MCP tool catalog with V6-friendly identifiers, titles, and metadata so that discovery endpoints can serve both runtimes without duplicating schemas.【F:.dev/src/mcp-server/v6-tool-adapter.ts†L194-L213】

In addition, `describeToolBridging()` surfaces the exact legacy↔︎V6 mapping for diagnostics and documentation so that downstream teams can confirm which tool identifiers are currently supported.【F:.dev/src/mcp-server/v6-tool-adapter.ts†L219-L222】
