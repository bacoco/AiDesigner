...
program
  .version(version)
  .description('aidesigner installer - Universal AI agent framework for any domain');

program
  .command('install')
  .description('Install aidesigner agents and tools')
  .option('-f, --full', 'Install complete aidesigner experience')
...
program
  .command('update')
  .description('Update existing aidesigner installation')
...
program
  ...
  .description('Check for aidesigner update')
  .action(async () => {
...
╚═╝  ╚═╝  ╚═════╝ ╚═╝╚══════╝ ╚══════╝ ╚═╝  ╚═╝╚══════╝
  ...
  const { directory } = await inquirer.prompt([
    {
      ...
      name: 'directory',
      message:
        'Enter the full path to your project directory where aidesigner should be installed:',
  ...
  const coreShortTitle = coreConfig['short-title'] || 'aidesigner Core System';

  // Add aidesigner core option
  let coreOptionText;
  if (state.type === 'v4_existing') {
    ...
    coreOptionText = `Update ${coreShortTitle} ${versionInfo} (.aidesigner-core)`;
  } else {
    coreOptionText = `${coreShortTitle} (v${version}) (.aidesigner-core)`;
  }

  choices.push({
    name: coreOptionText,
    value: 'aidesigner-core',
    checked: true,
  });
  ...
  // Process selections
  answers.installType = selectedItems.includes('aidesigner-core') ? 'full' : 'expansion-only';
  answers.expansionPacks = selectedItems.filter((item) => item !== 'aidesigner-core');

  // Ask sharding questions if installing aidesigner core
  if (selectedItems.includes('aidesigner-core')) {
  ...
  if (ides.includes('github-copilot')) {
    ...
    console.log(
      chalk.dim(
        'aidesigner works best with specific VS Code settings for optimal agent experience.\n',
      ),
    );
  ...
  if (ides.includes('opencode')) {
    ...
    const { useAgentPrefix, useCommandPrefix } = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'useAgentPrefix',
        message: "Prefix agent keys with 'aidesigner-'? (e.g., 'aidesigner-dev')",
        default: true,
      },
      {
        type: 'confirm',
        ...
        message:
          "Prefix command keys with 'aidesigner:tasks:'? (e.g., 'aidesigner:tasks:create-doc')",
        default: true,
      },
    ]);
    ...
    answers.openCodeConfig = {
      ...
      selectedPackages: {
        includeCore: selectedItems.includes('aidesigner-core'),
  ...
  if (ides.includes('auggie-cli')) {
    ...
    console.log(chalk.dim('Choose where to install aidesigner agents for Auggie CLI access.\n'));
