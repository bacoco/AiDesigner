...
import { flashCommands } from "./flash-commands.js";
import { getStorage } from "./design-library-storage.js";
import { getChromeMCPAdapter } from "./chrome-mcp-adapter.js";
import { PatternRemixService } from "./quick-designer-integration-v4.js";
...
class QuickDesignerMCPServer {
  ...
  private chromeMCP = getChromeMCPAdapter();
  private patternService = new PatternRemixService();
    ...
    return [
      {
        ...
        inputSchema: {
          ...
          properties: {
            request: {
              type: "string",
              description: "Description de l'écran à générer (ex: 'login screen minimal', 'dashboard modern')",
            },
            referenceUrl: {
              type: "string",
        ...
        inputSchema: {
          ...
          properties: {
            screenType: {
              ...
              enum: ["login", "dashboard", "pricing", "landing", "signup", "settings", "profile", "checkout", "search"],
            },
            style: {
              type: "string",
  ...
  private async handleToolCall(name: string, args: any): Promise<any> {
    switch (name) {
      ...
      case "quick_designer_refine":
        return await flashCommands.refine.handler(args);

      case "quick_designer_validate":
        return await flashCommands.validate.handler(args);

      case "quick_designer_show_system":
        return await flashCommands.showSystem.handler(args);

      case "quick_designer_open_mockup":
        return await flashCommands.openMockup.handler(args);
  ...
  private async handleInstant(args: any): Promise<any> {
    const result = await flashCommands.instant.handler(args);
  ...
  private async handleGenerateWithAI(args: any): Promise<any> {
  ...
  private async handleBatchGenerate(args: any): Promise<any> {
    ...
    for (const screen of screens) {
      const result = await flashCommands.instant.handler({
