...
type HtmlTemplatesModule = typeof import("../../lib/html-templates/index.js");
const { generateHTML, getVariationNames, isScreenTypeSupported } = requireLibModule<HtmlTemplatesModule>(
  "html-templates/index.js"
);
 ...
 */
async function generateMockupHTML(mockupData: any): Promise<string> {
...
<body>
  <nav class="tabs">
    ...
    ${pages.map((p: any, i: number) =>
      `<button data-page="${p.name}" ${p.selectedVariation ? 'class="validated' + (i === 0 && pages.length > 0 ? ' active' : '') + '"' : (i === 0 && pages.length > 0 ? 'class="active"' : '')}>${p.name.charAt(0).toUpperCase() + p.name.slice(1)}</button>`
    ).join('\n    ')}
  </nav>
  ...
  <div id="page-design-system" class="page ${pages.length === 0 ? 'active' : ''}">
    ...
    <div class="design-system-panel">
      ...
      <div class="spec-section">
        <h3>Couleurs</h3>
        <div>
          ${Object.entries(designSystem.colors).map(([key, val]: [string, any]) => {
            if (Array.isArray(val)) {
              return val.map((v: string) => `<span class="color-swatch" style="background: ${v}" title="${key}: ${v}"></span>`).join('');
            }
      ...
      <div class="spec-section">
        ...
        ${designSystem.typography.sizes ? `<p><strong>Tailles:</strong> ${Object.entries(designSystem.typography.sizes).map(([k, v]) => `${k}: ${v}`).join(', ')}</p>` : ''}
      </div>
      ` : ''}
      ...
      <div class="spec-section">
        ...
        <ul style="list-style: none; padding: 0;">
          ${pages.map((p: any) => `<li style="padding: 0.5rem 0;">${p.selectedVariation ? '✓' : '⏳'} ${p.name.charAt(0).toUpperCase() + p.name.slice(1)} ${p.selectedVariation ? `(Variation ${p.selectedVariation})` : '(en cours)'}</li>`).join('')}
        </ul>
      </div>
...
`,
        ...
        case "update_mockup": {
          const params = args as {
            action: "add_page" | "select_variation" | "update_design_system";
            page?: any;
            variationId?: number;
            pageName?: string;
            designSystem?: any;
          ...
          try {
            ...
            if (params.action === "add_page" && params.page) {
              ...
              if (params.page.variations && isScreenTypeSupported(params.page.type || params.page.name)) {
                const variationNames = getVariationNames(params.page.type || params.page.name);

                params.page.variations = params.page.variations.map((variation: any, index: number) => {
                  if (!variation.html && variation.specs) {
                    ...
                    try {
                      ...
                      console.warn(`Failed to generate HTML for ${params.page.name} - ${varName}:`, error);
            ...
            } else if (params.action === "select_variation" && params.pageName && params.variationId) {
              // Mark variation as selected
              const pageIndex = mockupData.pages.findIndex(
                (p: any) => p.name === params.pageName
              );
              if (pageIndex >= 0) {
                ...
                if (mockupData.pages[pageIndex].variations) {
                  const selectedVar = mockupData.pages[pageIndex].variations.find(
                    (v: any) => v.id === params.variationId
                  );
                  if (selectedVar && selectedVar.specs) {
                    mockupData.designSystem = {
                      ...mockupData.designSystem,
                      ...selectedVar.specs,
                      version: (parseFloat(mockupData.designSystem.version || "1.0") + 0.1).toFixed(1),
