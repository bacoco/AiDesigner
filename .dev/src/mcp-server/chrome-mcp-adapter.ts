...
export class ChromeMCPAdapter {
   ...
   */
  async extractFromURL(url: string): Promise<ExtractedDesign> {
    try {
   ...
   */
  private async executeChromeMCP(params: any): Promise<any> {
   ...
   */
  private simulateExtraction(url: string): any {
   ...
   */
  private convertToDesignSpec(extracted: any): DesignSpec {
    ...
    const accent = colors.find((c: string) => c !== primary && this.isAccentColor(c)) || "#10B981";

    // Extract neutral colors
    const neutrals = colors.filter((c: string) => this.isNeutralColor(c)).slice(0, 4);
    if (neutrals.length === 0) {
      neutrals.push("#6B7280", "#9CA3AF");
    }

    // Process fonts
    const fontFamily = fonts.filter((f: string) => !f.includes("emoji")).join(", ") || "system-ui";
   ...
   */
  private extractComponents(extracted: any): any {
   ...
   */
  private isAccentColor(color: string): boolean {
   ...
   */
  private findMostCommonValue(values: string[]): string | null {
   ...
   */
  private getFallbackDesign(url: string): ExtractedDesign {
    return {
      source: url,
      type: "url",
      timestamp: new Date().toISOString(),
   ...
   */
  async extractFromScreenshot(imagePath: string): Promise<ExtractedDesign> {
...
// Singleton instance
let adapterInstance: ChromeMCPAdapter | null = null;

export function getChromeMCPAdapter(): ChromeMCPAdapter {
  if (!adapterInstance) {
