...
class LLMClient {
  ...
  getApiKeyFromEnv() {
    switch (this.provider) {
      ...
      case 'glm': {
        return process.env.AGILAI_GLM_API_KEY || process.env.BMAD_GLM_API_KEY || process.env.GLM_API_KEY || process.env.ZHIPUAI_API_KEY;
      }
      default: {
  ...
  async chatGLM(messages, options) {
    ...
    const baseUrl =
      process.env.AGILAI_GLM_BASE_URL || process.env.BMAD_GLM_BASE_URL || process.env.GLM_BASE_URL || 'https://open.bigmodel.cn';
